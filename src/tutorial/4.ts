...
GTT.Factories.GDAX.FeedFactory(logger, products).then((feed: GDAXFeed) => {
  ...
  const streams = products.map(
    product => new GTT.Core.ProductFilter({ logger: logger, productId: product })
  );
  ...
  const fxGBP = new GTT.Core.ExchangeRateFilter({
    ...commonFilterConfig,
    pair: { from: 'GBP', to: 'USD' },
  });
  const fxEUR = new GTT.Core.ExchangeRateFilter({
    ...commonFilterConfig,
